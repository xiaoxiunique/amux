...
pub struct TimerDraft {
    /// Free text like `30m`, `2h`, `90s` — parsed by [`timer_parse_every`].
    pub every: String,
    pub prompt: String,
...
pub fn timer_parse_every(raw: &str) -> Option<u32> {
...
pub fn timer_every_label(secs: u32) -> String {
...
pub struct AppState {
    ...
    pub timer_editing: Option<TimerDraft>,
...
impl AppState {
    ...
    pub fn begin_timer(&mut self) -> bool {
        ...
        self.timer_editing = Some(TimerDraft {
            every: existing
                .as_ref()
                .map(|c| timer_every_label(c.every_secs))
        ...
        self.cursor = crate::store::setting(remembered::CURSOR)
            .and_then(|name| {
                self.rows().iter().position(|row| match row {
                    ...
                    Row::Project { index, .. } => {
                        self.project_at(*index).is_some_and(|p| p.dir == name)
                    }
                    Row::Draft => false,
                })
            })
    ...
    pub fn save_arrangement(&self) {
        ...
        let here = match self.current_row() {
            ...
            Some(Row::Project { index, .. }) => self.project_at(index).map(|p| p.dir.clone()),
            _ => None,
        };
    ...
    pub fn prune_browsing(&mut self) {
        let Some(name) = self.browsing_session.as_ref() else {
            return;
        };
        let live = self
            .projects
        ...
        match self.current_row() {
            ...
            Some(Row::Session { project, .. }) => {
                ...
                if let Some(row) = self
                    .rows()
                    .iter()
                    .position(|r| matches!(r, Row::Project { index, .. } if *index == project))
                {
                    self.cursor = row;
                }
            }
...
impl LiveTerm {
    ...
    fn spawn(label: String, mut command: CommandBuilder, cols: u16, rows: u16) -> Option<Self> {
...
fn copy_selection(live: &[LiveTerm], selection: Selection) -> Option<String> {
    ...
    let text = term
        .parser
        .screen()
        .contents_between(r1, c1, r2, c2.saturating_add(1));
...
fn event_loop(state: &mut AppState) -> Result<Outcome> {
    ...
    let result = loop {
        ...
        if live.iter().map(|t| &t.session).ne(wanted.iter()) {
            ...
            let mut spare: Vec<Option<LiveTerm>> =
                std::mem::take(&mut live).into_iter().map(Some).collect();
            live = wanted
                .iter()
                .enumerate()
                .filter_map(|(i, name)| {
        ...
        while let Ok((session, label)) = name_rx.try_recv() {
            match label {
                ...
                None => {
                    state.notice = Some(format!("could not name {session} (DeepSeek key set?)"));
                }
            }
            dirty = true;
        }
        ...
        if event::poll(POLL)? {
                                ...
                                if let Some(name) = name {
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        if let Some(label) = &label {
                                            let _ =
                                                crate::serve::sessions::set_label(&session, label);
                                        }
                ...
                if inside {
                    ...
                    match kind {
                        ...
                        MouseEventKind::Down(MouseButton::Left) => {
                            ...
                            let on_button =
                                tree_button_rects(tree).into_iter().find(|(rect, _, _)| {
                                    row == rect.y
                                        && column >= rect.x
                                ...
                                match button {
                                    ...
                                    TreeButton::Respawn => {
                                        if let Some(name) = state.current_name() {
                                            let found = state.projects.iter().find_map(|project| {
                                                project
                                                    .sessions
                                                    .iter()
                                                    .find(|s| s.name == name)
                                                    .map(|s| (project.dir.clone(), s.alias.clone()))
                                            });
                                            if let Some((dir, alias)) = found {
                                                let tx = spawn_tx.clone();
                                                let agents = state.agents.clone();
                                                state.notice = Some(format!("restarting {name}…"));
                                                std::thread::spawn(move || {
            ...
            if let Event::Key(key) = ev {
                        ...
                        match (state.current_name(), fields) {
                            (Some(name), Some((goal, turns, allow_waiting))) => {
                                ...
                                if crate::store::auto_enable(&name, &goal, turns, allow_waiting) {
                                    ...
                                    state.scheduled = crate::store::timer_enabled_sessions();
                ...
                if state.settings {
                    match key.code {
                        ...
                        KeyCode::Char('s') => match crate::serve::start_quiet() {
                            Ok(pid) => state.notice = Some(format!("monitor started (pid {pid})")),
                        ...
                        Some(Draft::Project(picker)) => match key.code {
                            ...
                            KeyCode::Enter => {
                                if let Some(entry) = picker.selected().cloned() {
                                    state.draft =
                                        Some(Draft::Session(SessionPicker::from_projects(
                                            entry.path.clone(),
                                            entry.label,
                                        )));
                ...
                match key.code {
                    ...
                    KeyCode::Char('G') | KeyCode::End => state.cursor = state.last_selectable(),
                    ...
                    KeyCode::Char('N') => {
                        ...
                        let agent = state.current_session().and_then(|s| {
...
fn columns_of(area: Rect) -> std::rc::Rc<[Rect]> {
    Layout::default()
        ...
        .constraints([
            Constraint::Length(tree_width(area.width)),
            Constraint::Min(1),
        ])
        .split(area)
}
...
pub fn encode_mouse(kind: MouseEventKind, col: u16, row: u16) -> Vec<u8> {
    let button = match kind {
        ...
        MouseEventKind::Up(_) => return format!("\x1b[<0;{};{}m", col + 1, row + 1).into_bytes(),
        _ => return Vec::new(),
    };
...
/// The schedule form: how often, and what to send.
fn render_timer_form(f: &mut Frame, state: &AppState, area: Rect) {
    let Some(draft) = state.timer_editing.as_ref() else {
        return;
    };
                ...
                rows.push(Line::from(vec![
                    Span::styled(
                        format!(
                            "{}{:<16}",
                            if selected { "> " } else { "  " },
                            truncate(&entry.label, 15)
                        ),
                        style,
                    ),
                    Span::styled(
                        elide_front(
                            &shorten_home(&entry.path),
                            width.saturating_sub(22) as usize,
                        ),
                        Style::default().fg(Color::DarkGray),
                    ),
                ]));
...
fn render_tree(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let items: Vec<ListItem> = rows
        ...
        .map(|row| match *row {
            ...
            Row::Draft => {
                let label = state.draft.as_ref().map(Draft::label).unwrap_or_default();
    ...
    if let Some((usage, area)) = footer {
        f.render_widget(
            Paragraph::new(Line::styled(
                format!(
                    "  {}",
                    crate::usage::summarise(usage, area.width.saturating_sub(2))
                ),
                Style::default().fg(Color::DarkGray),
            )),
            area,
...
fn pane_title(state: &AppState, live: Option<&LiveTerm>) -> String {
    match live
        ...
        .or(state.current_name().as_deref())
    {
...
fn render_terminal(f: &mut Frame, state: &AppState, live: &[LiveTerm], area: Rect) {
    ...
    if rects.len() <= 1 {
        render_one_terminal(
            ...
            state,
            live.first(),
            rects.first().copied().unwrap_or(area),
        );
        return;
    }
...
fn render_one_terminal(f: &mut Frame, state: &AppState, live: Option<&LiveTerm>, area: Rect) {
    ...
    match live {
        ...
        Some(term) => f.render_widget(PseudoTerminal::new(term.parser.screen()).block(block), area),
...
mod tests {
    ...
    fn projects_start_open() {
        ...
        assert_eq!(
            rows.len(),
            5,
            "sessions should be visible without pressing l"
        );
    ...
    fn wheel_events_encode_as_sgr_reports() {
        ...
        assert_eq!(
            encode_mouse(MouseEventKind::ScrollUp, 0, 0),
            b"\x1b[<64;1;1M"
        );
        assert_eq!(
            encode_mouse(MouseEventKind::ScrollDown, 4, 9),
            b"\x1b[<65;5;10M"
        );
    ...
    fn the_status_line_says_which_keys_are_live() {
        ...
        for focus in [Column::Tree, Column::Terminal] {
            ...
            assert!(
                text.contains("~ keys"),
                "{focus:?} does not point at the key list"
            );
        }
    }
    ...
    fn a_folded_project_still_shows_a_waiting_child() {
        ...
        assert!(
            !text.contains("0m"),
            "invented a duration for an inferred status"
        );
    }
    ...
    fn a_named_project_is_shown_and_sorted_by_its_name() {
        ...
        assert!(
            !text.contains("alpha"),
            "folder name drawn instead of the alias"
        );
    ...
    fn rows_name_the_agent_rather_than_its_alias() {
        ...
        assert!(
            ...
            "agent name truncated or not shown"
        );
    ...
    fn a_device_query_is_answered_once_however_it_is_split() {
        ...
        let mut carry = Vec::new();
        assert_eq!(
            count_attribute_queries(&mut carry, b"\x1b[2J\x1b[1;1Hhello"),
            0
        );
    }
    ...
    fn the_usage_footer_takes_a_row_from_the_list() {
        ...
        assert!(
            !text.contains("amux  "),
            "no reading yet, so nothing to show"
        );
    ...
    fn a_paste_lands_in_the_open_field_and_nowhere_otherwise() {
        ...
        let Some(Draft::Project(p)) = state.draft.as_ref() else {
            panic!()
        };
        ...
        let Some(Draft::Session(p)) = state.draft.as_mut() else {
            panic!()
        };
        p.filtering = true;
        assert!(paste_into_field(&mut state, "xhs"));
        let Some(Draft::Session(p)) = state.draft.as_ref() else {
            panic!()
        };
    ...
    fn a_position_maps_to_the_pane_it_is_over() {
        ...
        assert!(
            row < 20 && col < 50,
            "reported as {row},{col} — the column's origin, not the pane's"
        );
    ...
    fn the_esc_button_sends_a_real_escape() {
        assert!(PANE_BUTTONS
            .iter()
            .any(|button| matches!(button.action, PaneAction::Key(KeyCode::Esc, _))));
    ...
    fn the_pane_draws_its_controls() {
        ...
        assert!(
            !text.contains("AUTO"),
            "an unarmed session should not read AUTO"
        );
    ...
    fn panes_that_are_still_wanted_are_not_reconnected() {
        ...
        assert_eq!(
            at(&held, "d"),
            None,
            "a session nobody holds has to be opened"
        );
    ...
    fn an_interval_reads_the_way_it_is_written() {
        assert_eq!(timer_parse_every("30m"), Some(1800));
        assert_eq!(timer_parse_every("2h"), Some(7200));
        assert_eq!(timer_parse_every("90s"), Some(90));
        ...
        assert_eq!(timer_parse_every("5"), Some(300));
        assert_eq!(timer_parse_every("45"), Some(2700));
        ...
        assert_eq!(timer_parse_every("30m2m"), None);
        assert_eq!(timer_parse_every("soon"), None);
        ...
        assert_eq!(timer_parse_every("2m30"), None);
        assert_eq!(timer_parse_every(""), None);
        assert_eq!(timer_parse_every("2m"), Some(120));
        assert_eq!(timer_parse_every(" 15 M "), Some(900));

        // Under the floor is lifted to it rather than refused.
        assert_eq!(
            timer_parse_every("5s"),
            Some(crate::serve::server::MIN_EVERY_SECS)
        );
        assert_eq!(
            timer_parse_every("0m"),
            Some(crate::serve::server::MIN_EVERY_SECS)
        );
        // And a day is the ceiling.
        assert_eq!(timer_parse_every("99h"), Some(24 * 60 * 60));

        // Unreadable falls back rather than trapping the form.
        assert_eq!(timer_parse_every(""), None);
        assert_eq!(timer_parse_every("soon"), None);
        assert_eq!(timer_parse_every("10 fortnights"), None);

        // What the form shows back is what it parsed.
        assert_eq!(timer_every_label(1800), "30m");
        assert_eq!(timer_every_label(7200), "2h");
        assert_eq!(timer_every_label(90), "90s");
    }
    ...
    fn a_stale_listing_is_not_applied() {
        ...
        assert!(
            p.entries.is_empty(),
            "another project's sessions were shown"
        );
    ...
    fn the_hints_name_enter_and_the_fullscreen_key() {
        ...
        assert!(
            text.contains("Enter/i open"),
            "Enter is not described as opening"
        );
        assert!(
    ...
    fn cancelling_a_draft_restores_the_cursor() {
        ...
        assert!(
            state.cursor_before_draft.is_none(),
            "the saved cursor leaked"
        );
    }
    ...
    fn the_picker_hides_directories_that_are_gone() {
        ...
        let rows = vec![
            ...
            row(
                &tmp.path().join("deleted").to_string_lossy(),
                "deleted",
                None,
            ),
        ];
    ...
    fn refresh_auto_sessions_reads_what_is_stored() {
        ...
        assert!(crate::store::auto_enable(
            "cc_alpha_11111111",
            "goal-x",
            4,
            false
    ...
    fn the_browser_takes_the_column_and_says_how_to_leave() {
        ...
        assert!(
            text.contains("q quits yazi"),
            "yazi's own quit is not shown"
        );
    ...
    fn pinned_panes_take_the_left_and_browsing_keeps_the_rest() {
        ...
        let browse = three[3];
        assert!(
            browse.x >= 50 && browse.y >= 20,
            "browsing belongs bottom-right"
        );
    ...
    fn a_click_reaches_the_pane_it_landed_in() {
        ...
        for (want, r) in rects.iter().enumerate() {
            ...
            let got = rects
                .iter()
                .position(|q| cx >= q.x && cx < q.x + q.width && cy >= q.y && cy < q.y + q.height);
    ...
    fn navigation_works_from_either_column() {
        ...
        for focus in [Column::Tree, Column::Terminal] {
            ...
            let text = drawn(&state, 140);
            assert!(
                text.contains("hjkl move"),
                "{focus:?} does not offer navigation"
            );
            assert!(
                text.contains("JK scroll"),
                "{focus:?} does not offer scrolling"
            );
        }
    }
    ...
    fn an_empty_tree_says_how_to_fill_it() {
        ...
        assert!(
            ...
            "the empty state is silent"
        );
        assert!(
            text.contains("start an agent here"),
    ...
    fn renaming_shows_what_is_being_typed() {
        ...
        let text = drawn(&state, 140);
        assert!(
            text.contains("adobe-test"),
            "the text being typed is hidden"
        );
    ...
    fn the_tree_shows_projects_and_their_open_sessions() {
        ...
        assert!(
            ...
            "old three-column title survived"
        );
